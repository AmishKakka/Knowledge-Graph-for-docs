    ...
    def create_chunks(self, chunks: list):
        ...
        for idx, c in enumerate(chunks):
            chunks_with_metadata.append({
              ...
              'chunkId': str({c.metadata['page']})+'-'+str({idx})
            })
...
if __name__ == "__main__":
    ...
    # st = time()
    # document_splits = doc_loader.load_and_split("AttentionPaper.pdf")
    # et = time()
    # print("Total time taken: ", et-st)

    st = time()
    document_splits_lazy = doc_loader.lazy_load_and_split("AttentionPaper.pdf")
    et = time()
    print("Total time taken: ", et-st)
  
    print("\nTotal document splits: ", len(document_splits_lazy))

    chunks = doc_loader.create_chunks(document_splits_lazy)